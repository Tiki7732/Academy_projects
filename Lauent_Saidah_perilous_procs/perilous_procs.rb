...
def filter_out!(arr, &prc)
    arr.map! { |ele| ele if !prc.call(ele) }.compact!
end
...
def proc_suffix(sent, proc_hash)
    ...
    sent.split(" ").each do |ele|
        new_word = ""
        new_word += ele
        proc_hash.each do |key, val|
            new_word += val if key.call(ele)   
        end
         new_sent << new_word  
    end
    new_sent.join(" ")
...
def proctition_platinum(arr, *prcs)
    ...
    hashers = Hash.new{}
    prcs.each do |prc|
        new_arr = arr.select{|ele| ele if prc.call(ele)}
        arr.select!{|ele| ele if !prc.call(ele)}
        # p new_arr
        # p arr
        hashers[count] = new_arr
        count += 1
    end
    hashers
end
...
def picky_procipher(sent, proc_hash)
    ...
    sent.split(" ").each do |word|
        new_word = word
        proc_hash.reverse_each do |key, val|
            new_word = val.call(word)  if key.call(word) 
